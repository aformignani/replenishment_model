...
remove:{formButtons:{className:"btn-danger"}}});d.Editor.display.bootstrap=a.extend(!0,{},d.Editor.models.displayController,{init:function(e){var g=a('<div class="modal fade DTED"><div class="modal-dialog"><div class="modal-content"></div></div></div>'),l={content:g,close:a('<button class="close">&times;</div>').attr("title",e.i18n.close).on("click",function(){e.close("icon")}),modalContent:g.find("div.modal-content"),shown:!1},k=!1;a(c).on("mousedown","div.modal",function(h){l.shown&&(k=a(h.target).hasClass("modal")&&
l.shown?!0:!1)});a(c).on("click","div.modal",function(h){a(h.target).hasClass("modal")&&k&&e.background();a(h.target).hasClass("modal")&&(a("div.modal-dialog").height()>a(h.target).height()&&h.pageX>=c.body.offsetWidth-d.__browser.barWidth||e.background())});e.on("displayOrder.dtebs",function(h,n,p,q){a.each(e.s.fields,function(r,m){a("input:not([type=checkbox]):not([type=radio]), select, textarea",m.node()).addClass("form-control")})});e._bootstrapDisplay=l;return d.Editor.display.bootstrap},open:function(e,
g,l){var k=e._bootstrapDisplay;if(k.shown)l&&l();else{k.shown=!0;var h=k.modalContent;h.children().detach();h.append(g);a("div.modal-header",g).prepend(k.close);a(k.content).one("shown.bs.modal",function(){e.s.setFocus&&e.s.setFocus.focus();l&&l()}).one("hidden",function(){k.shown=!1}).appendTo("body").modal({backdrop:"static",keyboard:!1})}},close:function(e,g){e=e._bootstrapDisplay;e.shown&&(a(e.content).one("hidden.bs.modal",function(){a(this).detach()}).modal("hide"),e.shown=!1);g&&g()},node:function(e){return e._bootstrapDisplay.content[0]}});
return d.Editor});
